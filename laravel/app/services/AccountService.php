...
class AccountService extends BaseService implements GenericServices {
    ...
    public function authenticateAppUserByFacebook($credentials) {
        ...
        try {
        	...
        	$idonesignal = '';
        	if(isset($credentials["id_onesignal"]))$idonesignal = $credentials["id_onesignal"];
        	
        	$idfirebase = '';
        	if(isset($credentials["fcm_token_device"]))$idonesignal = $credentials["fcm_token_device"];
    ...
    public function create($input) {
        $user = new User();
        if(isset($input['complete_name']))$user->setCompleteName($input['complete_name']);
        ...
        if($this->userType === User::USER_TYPE_APP) {
        	  if(isset($input['id_onesignal']))$user->setIdOnesignal($input['id_onesignal']);
        	  if(isset($input['fcm_token_device']))$user->setIdFirebase($input['fcm_token_device']);
        	  if(isset($input['promo_code']))$user->setPromoCode($input['promo_code']);
        ...
        if(strlen($this->phoneBE)>1)$user->setCellPhone($this->phoneBE); /*Phone Admin*/
